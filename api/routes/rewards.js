...
// Marcar un reward como destacado (nivel 1, 2, 3)
...
// Obtener rewards pendientes de validación (solo admin)
router.get("/pendientes", async (req, res) => {
  try {
    const pendientes = await Reward.find({ validado: false }).sort({ createdAt: -1 });
    res.json(pendientes);
  } catch (err) {
