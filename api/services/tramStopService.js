...
function createTramStopService({
  provider = createZaragozaTramProvider(),
  now = () => Date.now(),
  ...
  function cachedResponse(entry, stale) {
    return { ...entry.value, fromCache: true, stale };
  }

  async function getStops() {
    if (stopsCache && now() - stopsCache.fetchedAt < stopsCacheTtlMs) {
    ...
    try {
      ...
      if (stopsCache && now() - stopsCache.fetchedAt <= STOPS_STALE_MAX_AGE_MS) {
  ...
  async function getArrivals(stopId) {
    ...
    if (cached && now() - cached.fetchedAt < arrivalsCacheTtlMs) {
    ...
    if (!inFlight) {
      inFlight = (async () => {
        ...
        if (result.notModified) {
          if (!cached) throw new Error("El origen devolvió 304 sin datos previos");
